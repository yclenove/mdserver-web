...
class stats:
    ...
    def disk(self):
        ...
        try:
            ...
            for disk_name in diskio_group.keys():
                ...
                # print(diskio_group[disk_name].write_time,
                #       diskio_cache[disk_name].write_time)
                # print(diskio_group[disk_name].write_count,
                #       diskio_cache[disk_name].write_count)

                diskInfo[disk_name]["read_count"] = int(
                    (
                        diskio_group[disk_name].read_count
                        - diskio_cache[disk_name].read_count
                    )
                    / stime
                )
                diskInfo[disk_name]["write_count"] = int(
                    (
                        diskio_group[disk_name].write_count
                        - diskio_cache[disk_name].write_count
                    )
                    / stime
                )
                diskInfo[disk_name]["read_bytes"] = int(
                    (
                        diskio_group[disk_name].read_bytes
                        - diskio_cache[disk_name].read_bytes
                    )
                    / stime
                )
                diskInfo[disk_name]["write_bytes"] = int(
                    (
                        diskio_group[disk_name].write_bytes
                        - diskio_cache[disk_name].write_bytes
                    )
                    / stime
                )
                diskInfo[disk_name]["read_time"] = int(
                    (
                        diskio_group[disk_name].read_time
                        - diskio_cache[disk_name].read_time
                    )
                    / stime
                )
                diskInfo[disk_name]["write_time"] = int(
                    (
                        diskio_group[disk_name].write_time
                        - diskio_cache[disk_name].write_time
                    )
                    / stime
                )
                ...
                if (
                    "read_merged_count" in diskio_group[disk_name]
                    and "read_merged_count" in diskio_cache[disk_name]
                ):
                    diskInfo[disk_name]["read_merged_count"] = int(
                        (
                            diskio_group[disk_name].read_merged_count
                            - diskio_cache[disk_name].read_merged_count
                        )
                        / stime
                    )
                if (
                    "write_merged_count" in diskio_group[disk_name]
                    and "write_merged_count" in diskio_cache[disk_name]
                ):
                    diskInfo[disk_name]["write_merged_count"] = int(
                        (
                            diskio_group[disk_name].write_merged_count
                            - diskio_cache[disk_name].write_merged_count
                        )
                        / stime
                    )
                ...
                if (
                    "read_merged_count" in diskInfo[disk_name]
                    and "read_merged_count" in diskInfo[disk_name]
                ):
                    diskInfo["ALL"]["read_merged_count"] += diskInfo[disk_name][
                        "read_merged_count"
                    ]
                if (
                    "write_merged_count" in diskInfo[disk_name]
                    and "write_merged_count" in diskInfo[disk_name]
                ):
    ...
    def network(self):
        ...
        for name in netio_group:
            ...
            if name not in netio_cache:
                netio_cache[name] = io_data

            netio_group_t[name] = io_data

            netInfo[name]["up"] = round(
                float((io_data[0] - netio_cache[name][0]) / stime), 2
            )
            netInfo[name]["down"] = round(
                float((io_data[1] - netio_cache[name][1]) / stime), 2
            )
