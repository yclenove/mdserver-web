...
def updateServer(stype, version=""):
    ...
    try:
        ...
        if stype == "update":
            ...
            newUrl = (
                "https://github.com/midoks/mdserver-web/archive/refs/tags/"
                + version
                + ".zip"
            )
...
cd /www/server/mdserver-web && pip3 install -r \
    /www/server/mdserver-web/requirements.txt -i $PIPSRC

P_VER_D=`echo "$P_VER"|awk -F '.' '{print $1}'`
P_VER_M=`echo "$P_VER"|awk -F '.' '{print $2}'`
NEW_P_VER=${P_VER_D}.${P_VER_M}

if [ -f /www/server/mdserver-web/version/r${NEW_P_VER}.txt \
];then
cd /www/server/mdserver-web && pip3 install -r \
    /www/server/mdserver-web/version/r${NEW_P_VER}.txt \
    -i $PIPSRC
fi
"""
            subprocess.run(update_env, shell=True, check=False)
            mw.restartMw()
