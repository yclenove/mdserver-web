...
def uncompress(sfile, dfile, path):
    ...
    if extension not in ["tar.gz", "gz", "zip", "rar", "7z", "xz", "bz2"]:
    ...
    try:
        ...
        if extension == "zip":
            cmd += (
                "&& unzip -o -d '" + dfile + "' '" + sfile + "' > " + tmps + " 2>&1 &"
            )
            mw.execShell(cmd)
        ...
        for key in json.loads(data):
            try:
                ...
                if not checkDir(filename):
                    return mw.returnData(False, "FILE_DANGER")
                subprocess.run(
                    ["chmod", "-R", access, filename], check=False
                )
                subprocess.run(
        ...
        for key in data:
            try:
                ...
                if os.path.isdir(filename):
                    ...
                    if key == ".user.ini":
                        subprocess.run(["chattr", "-i", filename], check=False)
                    if is_recycle:
                        mvRecycleBin(topath)
                    else:
                        os.remove(filename)
...
def setFileAccept(filename):
    ...
    if mw.getOs() == "darwin":
        ...
        auth = user + ":staff"
    subprocess.run(["chown", "-R", auth, filename], check=False)
    subprocess.run(["chmod", "-R", "755", filename], check=False)
            ...
            try:
                ...
                if el == "BIG5":
                    return mw.returnData(
                        False, "文件编码不被兼容，无法正确读取文件!" + str(ex)
                    )
    else:
    ...
    if ftype == "mtime":
        try:
            ...
            if sort == "desc":
                flist = sorted(
                    flist,
                    key=lambda f: os.path.getctime(os.path.join(path, f)),
                    reverse=True,
                )
            if sort == "asc":
                flist = sorted(
...
def setFileAccess(filename, user, access):
    ...
    try:
        ...
        subprocess.run(["chmod", sall, access, filename], check=False)
        subprocess.run(["chown", sall, user + ":" + user, filename], check=False)
        msg = mw.getInfo(
            "设置[{1}]权限为[{2}]所有者为[{3}]",
            (
                filename,
...
def closeLogs():
    ...
    subprocess.run(["rm", "-rf", log_file + "/*"], check=False)
    mw.opWeb("reload")
    # os.system('kill -USR1 `cat '
    #     + mw.getServerDir() + '/openresty/nginx/logs/nginx.pid`')
