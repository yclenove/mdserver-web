...
class crontab(object):
    ...
    def startTask(self, cron_id):
        ...
        subprocess.run(["chmod", "+x", cmd_file], check=False)
        subprocess.Popen(
            [cmd_file],
            stdout=open(cmd_file + ".log", "a"),
    ...
    def getCrontabHuman(self, data):
        ...
        for i in range(len(data)):
            ...
            if t["type"] == "day":
                ...
                t["cycle"] = mw.getInfo(
                    "每天, {1}点{2}分 执行",
                    (str(t["where_hour"]), str(t["where_minute"])),
                )
            elif t["type"] == "day-n":
                t["type"] = mw.getInfo("每{1}天", (str(t["where1"]),))
                t["cycle"] = mw.getInfo(
                    "每隔{1}天, {2}点{3}分 执行",
                    (str(t["where1"]), str(t["where_hour"]), str(t["where_minute"])),
                )
            elif t["type"] == "hour":
                t["type"] = "每小时"
                t["cycle"] = mw.getInfo(
                    "每小时, 第{1}分钟 执行", (str(t["where_minute"]),)
                )
            elif t["type"] == "hour-n":
                t["type"] = mw.getInfo("每{1}小时", (str(t["where1"]),))
                t["cycle"] = mw.getInfo(
                    "每{1}小时, 第{2}分钟 执行",
                    (str(t["where1"]), str(t["where_minute"])),
                )
    ...
    def getCrontabList(self, page=1, size=10):
        info = thisdb.getCrontabList(page=int(page), size=int(size))

        rdata = {}
        rdata["data"] = self.getCrontabHuman(info["list"])
        rdata["page"] = mw.getPage(
            {"count": info["count"], "tojs": "getCronData", "p": page, "row": size}
        )

        # backup hook
        rdata["backup_hook"] = thisdb.getOptionByJson(
            "hook_backup", type="hook", default=[]
        )
        return rdata
    ...
    def cronCheck(self, params):
        if (
            params["stype"] == "site"
            or params["stype"] == "database"
            or params["stype"].find("database_") > -1
            or params["stype"] == "logs"
            or params["stype"] == "path"
        ):
            if params["save"] == "":
    ...
    source $MW_PATH
            ...
            try:
                ...
                if stype == "toUrl":
                    shell = (
                        head
                        + "curl -sS --connect-timeout 10 -m 60 '"
                        + param["url_address"]
                        + "'"
                    )
                else:
