...
def getGlobalVar():
    ...
    data["site_path"] = thisdb.getOption(
        "site_path", default=mw.getFatherDir() + "/wwwroot"
    )
    data["backup_path"] = thisdb.getOption(
        "backup_path", default=mw.getFatherDir() + "/backup"
    )
    ...
    data["two_step_verification"] = thisdb.getOptionByJson(
        "two_step_verification", default={"open": False}
    )
    ...
    data["hook_global_static"] = thisdb.getOptionByJson(
        "hook_global_static", type="hook", default=[]
    )
    data["hook_database"] = thisdb.getOptionByJson(
        "hook_database", type="hook", default=[]
    )

    # 邮件通知设置
    data["notify_email"] = thisdb.getOptionByJson(
        "notify_email", default={"open": False}, type="notify"
    )
    data["notify_tgbot"] = thisdb.getOptionByJson(
        "notify_tgbot", default={"open": False}, type="notify"
    )
