...
def __to_date2(date_str):
    tmp = date_str.split()
    s_date = (
        str(tmp[-1]) + "-" + __months.get(tmp[1], tmp[1]) + "-" + tmp[2] + " " + tmp[3]
    )
    return s_date
...
def getAuditLast(log_name):
    # 获取日志
    cmd = (
        'LANG=en_US.UTF-8 last -n 200 -x -f {}'
        '|grep -v 127.0.0.1|grep -v " begins"'
    ).format("/var/log/" + log_name)
    ...
    for _line in result[0].split("\n"):
        ...
        if sp_arr[0] == "runlevel":
            ...
            if sp_arr[-3] == "-":
                tmp["时间"] = (
                    __to_date3(" ".join(sp_arr[4:])) + " " + " ".join(sp_arr[-3:])
                )
            else:
                tmp["时间"] = (
                    __to_date3(" ".join(sp_arr[4:])) + " " + " ".join(sp_arr[-2:])
                )
        elif (
            sp_arr[1] in ["tty1", "tty", "tty2", "tty3", "hvc0", "hvc1", "hvc2"]
            or len(sp_arr) == 9
        ):
            tmp["来源"] = ""
            tmp["端口"] = sp_arr[1]
    ...
    if log_name.find("sa/sa") == -1:
        if _line[:3] in __months:
            ...
            if len(_tmp) > 1:
                ...
                _msg = _tmp[0]
            _line = {"时间": __to_date4(_line[:16].strip()), "角色": _act, "事件": _msg}
            is_string = False
...
def getAuditLogsName(log_name):
    if (
        log_name in ["wtmp", "btmp", "utmp"]
        or log_name.find("wtmp") == 0
        or log_name.find("btmp") == 0
        or log_name.find("utmp") == 0
    ):
