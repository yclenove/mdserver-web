...
class sites(object):
    ...
    def __init__(self):
        ...
        if not os.path.exists(self.sslLetsDir):
            mw.execShell(
                "mkdir -p " + self.sslLetsDir + " && chmod -R 755 " + self.sslLetsDir
            )
    ...
    def saveHostConf(self, path, data, encoding):
        ...
        if rdata["status"]:
            ...
            if isError is not True:
                mw.restoreFile(path)
                msg = (
                    'ERROR: 检测到配置文件有错误,请先排除后再操作<br><br><a style="color:red;">'
                    + isError.replace("\n", "<br>")
                    + "</a>"
                )
    ...
    def add(self, site_info, port, ps, path, version):
        ...
        self.siteName = self.toPunycode(
            site_info["domain"].strip().split(":")[0]
        ).strip()
    ...
    def addDomain(self, site_id, site_name, domain):
        ...
        for d in domains:
            ...
            if thisdb.checkSitesDomainIsExist(name, port):
                return mw.returnData(
                    False, "您添加的域名[{}:{}],已使用。请仔细检查!".format(name, port)
                )
    ...
    def delDomain(self, site_id, site_name, domain, port):
        ...
        info = (
            mw.M("domain")
            .field("id,name")
            .where("pid=? AND name=? AND port=?", (site_id, domain, port))
            .find()
        )
        ...
        if conf:
            ...
            tmp = re.findall(rep, conf)
            port_nums = (
                mw.M("domain").where("pid=? AND port=?", (site_id, port)).count()
            )
            if mw.inArray(tmp, port) is True and port_nums < 2:
                rep = r"\n*\s+listen\s+" + port + ";"
...
"""
    ...
    ssl_ciphers ECDHE-ECDSA-AES128-GCM-SHA256:ECDHE-RSA-AES128-GCM-SHA256:ECDHE-ECDSA-AES256-GCM-SHA384:ECDHE-RSA-AES256-GCM-SHA384:ECDHE-ECDSA-CHACHA20-POLY1305:ECDHE-RSA-CHACHA20-POLY1305:DHE-RSA-AES128-GCM-SHA256:DHE-RSA-AES256-GCM-SHA384:DHE-RSA-CHACHA20-POLY1305;  # noqa: E501
    ...
    def getDirBinding(self, site_id):
        ...
        if not os.path.exists(path):
            ...
            if path in checks:
                ...
                data["binding"] = []
                return mw.returnData(True, "OK", data)
            subprocess.run(["mkdir", "-p", path], check=False)
            subprocess.run(["chmod", "755", path], check=False)
            subprocess.run(["chown", "www:www", path], check=False)
            siteName = info["name"]
            mw.writeLog(
                "网站管理",
                "站点[" + siteName + "],根目录[" + path + "]不存在,已重新创建!",
            )
    ...
    def addDirBind(self, site_id, domain, dir_name):
        ...
        msg = mw.getInfo(
            "网站[{1}]子目录[{2}]绑定到[{3}]", (info["name"], dir_name, domain)
        )
        mw.writeLog("网站管理", msg)
        mw.restartWeb()
    ...
    def getDirBindingRewrite(self, binding_id, add):
        ...
        if add == "1":
            ...
            tmp = re.search(rep, conf).group()
            dirConf = tmp.replace(
                "rewrite/" + info["name"] + ".conf;",
                "rewrite/" + info["name"] + "_" + binding_info["path"] + ".conf;",
            )
    ...
    def delDirBinding(self, binding_id):
        ...
        msg = mw.getInfo(
            "删除网站[{1}]子目录[{2}]绑定", (info["name"], binding_info["path"])
        )
        mw.writeLog("网站管理", msg)
        mw.restartWeb()
    ...
    def setRedirect(self, site_name, site_from, to, type, r_type, keep_path):
        ...
        if _type_code == 1:
            domain_list = (
                ...
                .field("id,pid,name,port,add_time")
                .select()
            )
            site_domain_lists = (
                mw.M("domain")
                .where("pid=?", (domain_list[0]["pid"],))
                .field("name")
                .select()
            )
            found = False
        ...
        rep = (
            r"http(s)?\:\/\/([a-zA-Z0-9][-a-zA-Z0-9]{0,62}\.)"
        ...
        data.append(
            {
                ...
                "r_type": _type_code,
                "r_to": to,
                "keep_path": _keep_path,
                "id": _id,
            }
        )
        mw.writeFile(redirect_file, json.dumps(data))
        mw.writeFile(
            "{}/{}.conf".format(self.getRedirectPath(site_name), _id), file_content
        )
    ...
    def delRedirect(self, siteName, rid):
        ...
        try:
            ...
            # remove conf file
            mw.execShell(
                "rm -rf {}/{}.conf".format(self.getRedirectPath(siteName), rid)
            )
    ...
    def setProxy(
        ...
        data_content = (
            mw.readFile(proxy_site_path) if os.path.exists(proxy_site_path) else ""
        )
        data = json.loads(data_content) if data_content != "" else []
...
"
        ...
        if rule_test is not True:
            os.remove(conf_proxy)
            return mw.returnData(
                False, "OpenResty配置测试不通过, 请重试: {}".format(rule_test)
            )
    ...
    def closeProxyAll(self, site_name):
        ...
        if os.path.exists(proxy_path):
            ...
            for proxy in data:
                ...
                proxy_txt = "{}/{}/{}.conf.txt".format(
                    self.proxyPath, site_name, proxy["id"]
                )
    ...
    def closeRedirectAll(self, site_name):
        ...
        if os.path.exists(redirect_path):
            ...
            for redirect_data in data:
                ...
                redirect_txt = "{}/{}/{}.conf.txt".format(
                    self.redirectPath, site_name, redirect_data["id"]
                )
    ...
    def openRedirectByOpen(self, site_name):
        for redirect_id in self.close_redirect:
            ...
            redirect_txt = "{}/{}/{}.conf.txt".format(
                self.redirectPath, site_name, redirect_id
            )
    ...
    def saveRedirectConf(self, site_name, redirect_id, config):
        ...
        _old_config = mw.readFile(
            "{}/{}/{}.conf".format(self.redirectPath, site_name, redirect_id)
        )
        if _old_config is False:
            return mw.returnData(False, "非法操作")

        mw.writeFile(
            "{}/{}/{}.conf".format(self.redirectPath, site_name, redirect_id), config
        )
        rule_test = mw.checkWebConfig()
        if rule_test is not True:
            mw.writeFile(
    ...
    def getSsl(self, site_name, ssl_type):
        ...
        if ssl_type == "lets":
            csr_path = (
                self.sslLetsDir + "/" + site_name + "/fullchain.pem"
            )  # Let生成证书路径
            key_path = (
                self.sslLetsDir + "/" + site_name + "/privkey.pem"
            )  # Let密钥文件路径
        elif ssl_type == "acme":
    ...
    def saveCert(self, site_name, keyPath, certPath):
        try:
            ...
            if not os.path.exists(vpath):
                subprocess.run(["mkdir", "-p", vpath], check=False)
    ...
    def setDefaultSite(self, name):
        ...
        if os.path.exists(path):
            ...
            rep = r"listen\s+\[\:\:\]\:443\s*quic\s*reuseport;"
            conf = re.sub(
                rep, "listen [::]:443 quic reuseport default_server;", conf, 1
            )
    ...
    def getDnsapi(self):
        ...
        dnsapi_option = [
            ...
            },
            {
                "name": "dns_cf",
                "title": "cloudflare",
                "key": "CF_Key:CF_Email:CF_Token:CF_Account_ID:CF_Zone_ID",
            ...
            },
            {
                "name": "dns_tencent",
                "title": "腾讯云DNS",
                "key": "Tencent_SecretId:Tencent_SecretKey",
                "help": "腾讯云后台获取通行证",
            },
            {"name": "dns_gd", "title": "GoDaddy", "key": "GD_Key:GD_Secret"},
            # {"name":"dns_pdns", "title":'PowerDNS',
            #  'key':'PDNS_Url:PDNS_ServerId:PDNS_Token:PDNS_Ttl'},
            ...
            # {"name":"dns_ispconfig", "title":'ISPConfig',
            #  'key':'ISPC_User:ISPC_Password:ISPC_Api:ISPC_Api_Insecure'},
            # {"name":"dns_ad", "title":'Alwaysdata', 'key':'AD_API_KEY'},
            {"name": "dns_linode_v4", "title": "Linode", "key": "LINODE_V4_API_KEY"},
            # {"name":"dns_freedns", "title":'FreeDNS',
            #  'key':'FREEDNS_User:FREEDNS_Password'},
            # {"name":"dns_cyon", "title":'cyon.ch',
            #  'key':'CY_Username:CY_Password:CY_OTP_Secret'},
            # {"name":"dns_gandi_livedns", "title":'LiveDNS',
            #  'key':'GANDI_LIVEDNS_TOKEN'},
            # {"name":"dns_knot", "title":'Knot', 'key':'KNOT_SERVER:KNOT_KEY'},
            {"name": "dns_dgon", "title": "DigitalOcean", "key": "DO_API_KEY"},
            # {"name":"dns_cloudns", "title":'ClouDNS.net',
            #  'key':'CLOUDNS_SUB_AUTH_ID:CLOUDNS_AUTH_PASSWORD'},
            {
                "name": "dns_namesilo",
                "title": "Namesilo",
                "key": "Namesilo_Key",
            },
            {
                ...
                "key": (
                    ...
                    ":AZUREDNS_APPID:AZUREDNS_CLIENTSECRET"
                ),
            },
            # {"name":"dns_selectel", "title":'selectel.com', 'key':'SL_Key'},
            # {"name":"dns_zonomi", "title":'zonomi.com', 'key':'ZM_Key'},
            # {"name":"dns_kinghost", "title":'KingHost',
            #  'key':'KINGHOST_Username:KINGHOST_Password'},
            # {"name":"dns_zilore", "title":'Zilore', 'key':'Zilore_Key'},
            {
                "name": "dns_gcloud",
                "title": "Google Cloud DNS",
                "key": "CLOUDSDK_ACTIVE_CONFIG_NAME",
            },
            # {"name":"dns_mydnsjp", "title":'MyDNS.JP',
            #  'key':'MYDNSJP_MasterID:MYDNSJP_Password'},
    ...
    def createAcmeFile(self, site_name, apply_ca, domains, email, force, renew):
        ...
        if renew == "true":
            cmd = (
                acme_dir
                + "/acme.sh --renew --yes-I-know-dns-manual-mode-enough-go-ahead-please"
            )
        else:
...
""" % (
    ...
    def createAcmeDns(
        ...
        # 手动方式申请
        if dnspai == "none":
            return self.createAcmeDnsTypeNone(
                site_name,
                domains,
                email,
                dnspai,
        ...
        if dnspai not in dnsapi_option:
...
""" % (
    ...
    def createAcme(
        ...
        if not os.path.exists(acme_dir):
            return mw.returnData(
                False,
                "尝试自动安装ACME失败,请通过以下命令尝试手动安装<p>安装命令: curl https://get.acme.sh | sh</p>",
            )
        ...
        if apply_type == "file":
            return self.createAcmeFile(
                site_name, apply_ca, domains, email, force, renew
            )
        elif apply_type == "dns":
            return self.createAcmeDns(
                site_name,
                apply_ca,
                domains,
                email,
        ...
        if os.path.exists(host_conf_file):
            ...
            if siteConf.find("301-END") != -1:
                return mw.returnJson(
                    False, "检测到您的站点做了301重定向设置，请先关闭重定向!"
                )

            # 检测存在反向代理
            data_path = self.getProxyDataPath(site_name)
            data_content = mw.readFile(data_path)
            if data_content is not False:
                try:
                ...
                for proxy in data:
                    ...
                    if os.path.exists(proxy_dir_file):
                        return mw.returnJson(
                            False, "检测到您的站点做了反向代理设置，请先关闭反向代理!"
                        )
    ...
    def getBackup(self, site_id, page=1, size=10):
        ...
        data["data"] = info["list"]
        data["site"] = site_info
        data["page"] = mw.getPage(
            {"count": info["count"], "tojs": "getBackup", "p": page, "row": size}
        )
        return data
    ...
    def delBackup(self, backup_id):
        ...
        msg = mw.getInfo(
            "删除网站[{1}]的备份[{2}]成功!", (info["name"], info["filename"])
        )
        mw.writeLog("网站管理", msg)
