...
class ssh_terminal(object):
    ...
    def connectLocalSsh(self, sid):
        ...
        try:
            ...
            if e.find("Error reading SSH protocol banner") != -1:
                ...
                return self.returnMsg(
                    False, "协议头响应超时，与目标服务器之间的网络质量太糟糕：" + e
                )
            if not e:
                self.debug("SSH协议握手超时")
                return self.returnMsg(
                    False, "SSH协议握手超时，与目标服务器之间的网络质量太糟糕"
                )
        ...
        ssh = self.__ps.invoke_shell(term="xterm", width=83, height=21)
        ssh.setblocking(0)
        self.__ssh_list[sid] = ssh
        mw.writeLog(
            self.__log_type,
            "成功登录到SSH服务器 [{}:{}]".format(self.__host, self.__port),
        )
    ...
    def connectBySocket(self, sid):
        ...
        while num < 5:
            ...
            try:
                ...
                if num == 5:
                    ...
                    return self.returnMsg(
                        False,
                        "连接目标服务器失败, {}:{}".format(self.__host, self.__port),
                    )
                else:
                    time.sleep(0.2)
        ...
        try:
            ...
            if not self.__pass and not self.__pkey:
                return self.returnMsg(
                    False,
                    "密码或私钥不能都为空: {}:{}".format(self.__host, self.__port),
                )
    ...
    def resize(self, sid, data):
        try:
            self.__ssh_list[sid].resize_pty(width=data["cols"], height=data["rows"])
            return True
        except Exception:
            return False
