...
class Firewall(object):
    ...
    def aIF_Ufw(self):
        ...
        for pinfo in ports_list:
            ...
            for i in range(len(ports_all)):
                if (
                    ports_all[i]["port"] == info[0]
                    and ports_all[i]["protocol"] != info[1]
                ):
        ...
        for add_info in ports_all:
            if thisdb.getFirewallCountByPort(add_info["port"]) == 0:
                thisdb.addFirewall(
                    add_info["port"], ps="自动识别", protocol=add_info["protocol"]
                )
    ...
    def getList(self, page=1, size=10):
        ...
        rdata["page"] = mw.getPage(
            {"count": info["count"], "tojs": "showAccept", "p": page, "row": size}
        )
        return rdata
    ...
    def setSshPort(self, port):
        ...
        if not self.reloadSshd():
            return mw.returnData(
                False, "重启sshd失败,尝试手动重启:service ssh restart!"
            )
    ...
    def addAcceptPortCmd(self, port, protocol="tcp"):
        if self.__isUfw:
            ...
            if protocol == "udp":
                cmd = (
                    "iptables -I INPUT -p udp -m state --state NEW -m udp --dport "
                    + port
                    + " -j ACCEPT"
                )
                mw.execShell(cmd)
    ...
    # 添加放行端口
    def addAcceptPort(self, port, ps, stype, protocol="tcp"):
    ...
    def delAcceptPort(self, firewall_id, port, protocol="tcp"):
    ...
    def delAcceptPortCmd(self, port, protocol="tcp"):
        if self.__isUfw:
            if protocol == "tcp":
            ...
            if protocol == "udp":
                mw.execShell(
                    "iptables -D INPUT -p udp -m state --state NEW -m udp --dport "
                    + port
                    + " -j ACCEPT"
                )
            if protocol == "tcp/udp":
