...
本模块是 MW-Linux 面板的核心工具库，提供了面板运行所需的基础功能。

主要功能分类:
    - 系统命令执行: execShell, checkBinExist
    - 路径管理: getPanelDir, getRootDir, getServerDir 等
    - 文件操作: readFile, writeFile, getPathSize 等
    - 加密解密: md5, aesEncrypt, aesDecrypt, enDoubleCrypt, deDoubleCrypt
    - 网络请求: HttpGet, HttpPost
    - 系统信息: getOs, getCpuType, getLocalIp 等
    - 日志管理: writeLog, writeFileLog, writeDbLog
    - 数据库操作: M (ORM 快捷方法)
    - 面板管理: restartMw, panelCmd, checkWebConfig 等
...
def execShell(cmdstring, cwd=None, timeout=None, shell=True):
    ...
    Example:
        >>> stdout, stderr = execShell('ls -la')
        >>> stdout, stderr = execShell('echo hello', shell=False)
    """
    cmd = cmdstring if shell else shlex.split(cmdstring)
    end_time = None
    if timeout:
        end_time = (
...
def getRecycleBinDir():
    ...
    if not os.path.exists(rb_dir):
        subprocess.run(["mkdir", "-p", rb_dir], check=False)
    return rb_dir
...
def sortAllFileList(path, ftype="mtime", sort="desc", search="", limit=3000):
    ...
    for d_list in os.walk(path):
        ...
        for d in d_list[1]:
            if count >= limit:
                break
            if not search_lower or search_lower in d.lower():
                filename = base_path + "/" + d
                count += 1
        ...
        for f in d_list[2]:
            if count >= limit:
                break
            if not search_lower or search_lower in f.lower():
...
def returnData(status, msg, data=None):
    ...
    Example:
        >>> returnData(True, 'success')
        {'status': True, 'msg': 'success'}
        >>> returnData(False, 'error', {'code': 404})
        {'status': False, 'msg': 'error', 'data': {'code': 404}}
    """
    if data is None:
...
def readFile(filename, use_cache=False):
    """读文件内容

    Args:
        filename: 文件路径
        use_cache: 是否使用缓存（适用于频繁读取的配置文件）
    ...
    if use_cache:
        ...
        if filename in _file_cache:
            if current_time - _file_cache_timeout.get(filename, 0) < CACHE_TTL:
    ...
    try:
        ...
        if use_cache:
            ...
            _file_cache_timeout[filename] = time.time()
...
def clearFileCache(filename=None):
    """清除文件缓存

    Args:
        filename: 指定文件路径，为 None 时清除所有缓存
    """
    if filename:
        _file_cache.pop(filename, None)
        _file_cache_timeout.pop(filename, None)
    else:
        _file_cache.clear()
        _file_cache_timeout.clear()
        ...
        for flag in ["-4", "-6"]:
            try:
                ...
                cmd = (
                    "curl --insecure "
                    + flag
                    + " -sS --connect-timeout 5 -m 60"
                    + " https://speed.cloudflare.com/cdn-cgi/trace"
                )  # 使用 speed.cloudflare.com/cdn-cgi/trace
                ip = execShell(cmd)
...
def getCpuType():
    ...
    if isAppleSystem():
        cmd = (
            ...
            " | grep 'Processor Name' | awk -F ':' '{print $2}'"
        )
...
def _read_lines_from_end(fp, count):
    """从文件末尾读取指定行数的内容
...
def getLastLine(path, num, p=1):
    ...
    try:
        ...
        start_line = (p - 1) * num
        total_lines = start_line + num

        with open(path, "rb") as fp:
            data = _read_lines_from_end(fp, total_lines)
            # 只返回当前页的行
            if start_line > 0 and len(data) > start_line:
...
def writeFileLog(msg, path=None, limit_size=50 * 1024 * 1024, save_limit=3):
    ...
    Args:
        msg: 日志消息
        path: 日志文件路径
        limit_size: 日志文件大小限制（字节）
        save_limit: 保留的历史日志文件数量

    Returns:
        True
    """
    log_file = path or (getPanelDir() + "/logs/debug.log")
        ...
        if os.path.exists(log_file):
            try:
                ...
                if size > limit_size:
                    ...
                    log_file_rename = log_file + "_" + ts + ".log"
                    os.rename(log_file, log_file_rename)
                    logs = sorted(glob.glob(log_file + "_*"))
                    count = len(logs)
                    for i in range(max(0, count - save_limit)):
                        try:
                            os.remove(logs[i])
                        except OSError:
                            pass
...
def enDoubleCrypt(key, strings):
    ...
    Example:
        >>> encrypted = enDoubleCrypt('mykey', 'secret data')
    """
    try:
        import base64
...
def deDoubleCrypt(key, strings):
    ...
    Example:
        >>> decrypted = deDoubleCrypt('mykey', encrypted_data)
    """
    try:
        import base64
...
def aesEncrypt(data, key="ABCDEFGHIJKLMNOP", vi="0102030405060708"):
    ...
    Example:
        >>> encrypted = aesEncrypt('hello world', '1234567890123456')
    """
    ...
    cipher = Cipher(
        algorithms.AES(AES_CBC_KEY), modes.CBC(AES_CBC_IV), backend=default_backend()
    )
...
def aesDecrypt(data, key="ABCDEFGHIJKLMNOP", vi="0102030405060708"):
    ...
    Args:
        data: 被解密的数据（字节）
        key: 解密密钥（16字节，需与加密时相同）
        vi: 初始化向量（16字节，需与加密时相同）
    ...
    Example:
        >>> decrypted = aesDecrypt(encrypted_data, '1234567890123456')
    """
...
def HttpGet(url, timeout=10):
    ...
    Example:
        >>> content = HttpGet('https://example.com/api/data')
    """
    try:
...
def HttpPost(url, data, timeout=10):
    ...
    Example:
        >>> result = HttpPost('https://example.com/api', {'key': 'value'})
    """
...
def restartTask():
    ...
    if os.path.exists(initd):
        subprocess.run([initd, "restart_task"], check=False)
    return True
...
def opWeb(method):
    ...
    if os.path.exists(sys_initd):
        subprocess.run([sys_initd, method], check=False)
        return True
...
def createRsa():
    ...
    if not os.path.exists(ssh_dir + "/id_rsa.pub") and os.path.exists(
        ssh_dir + "/id_rsa"
    ):
...
def createSshInfo():
    ssh_dir = getSshDir()
    if not os.path.exists(ssh_dir + "/id_rsa") or not os.path.exists(
        ssh_dir + "/id_rsa.pub"
    ):
        createRsa()
...
def tgbotNotifyObject():
    ...
    if "tgbot" in data and "enable" in data["tgbot"]:
        if data["tgbot"]["enable"]:
            tgbot_data = data["tgbot"]["data"]
            import telebot

            bot = telebot.TeleBot(tgbot_data.get("app_token", ""))
            return True, bot
    return False, None
