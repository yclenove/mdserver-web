...
# 日志页面
blueprint = Blueprint(
    "logs", __name__, url_prefix="/logs", template_folder="../../templates"
)
...
def get_log_list():
    ...
    data["page"] = mw.getPage(
        {"count": info["count"], "tojs": "getLogs", "p": p, "row": size}
    )
    return data
...
# 系统审计日志列表
@blueprint.route(
    "/get_audit_logs_files", endpoint="get_audit_logs_files", methods=["POST"]
)
@panel_login_required
