...
def get_temp_login():
    ...
    data["page"] = mw.getPage(
        {"count": info["count"], "tojs": "setTempAccessReq", "p": p, "row": limit}
    )
    return data
...
def set_temp_login():
    ...
    if r > 0:
        mw.writeLog(
            "面板设置", "生成临时连接,过期时间:{}".format(mw.formatDate(times=expire))
        )
        return {
            "status": True,
            "msg": "临时连接已生成",
...
@blueprint.route(
    "/get_temp_login_logs", endpoint="get_temp_login_logs", methods=["POST"]
)
@panel_login_required
