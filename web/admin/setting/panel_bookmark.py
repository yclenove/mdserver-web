...
def add_panel_info():
    ...
    isRe = mw.M("panel").add(
        "title,url,username,password,click,add_time",
...
def get_panel_list():
    data = (
        mw.M("panel")
        .field("id,title,url,username,password,click,add_time")
        .order("click desc")
        .select()
    )
...
def set_panel_info():
    ...
    isSave = (
        mw.M("panel")
        .where("(title=? OR url=?) AND id!=?", (title, url, panel_id))
        .count()
    )
    if isSave:
        return mw.returnData(False, "备注或面板地址重复!")
    ...
    isRe = (
        mw.M("panel")
        .where("id=?", (panel_id,))
        .save("title,url,username,password", (title, url, username, password))
    )
    if isRe:
