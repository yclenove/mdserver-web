...
blueprint = Blueprint(
    "site", __name__, url_prefix="/site", template_folder="../../templates/default"
)
...
def list():
    ...
    info = thisdb.getSitesList(
        page=int(p), size=int(limit), type_id=int(type_id), search=search, order=order
    )

    data = {}
    data["data"] = info["list"]
    data["page"] = mw.getPage(
        {"count": info["count"], "tojs": "getWeb", "p": p, "row": limit}
    )
    return data
...
# 获取站点PHP版本
@blueprint.route(
    "/get_site_php_version", endpoint="get_site_php_version", methods=["POST"]
)
@panel_login_required
