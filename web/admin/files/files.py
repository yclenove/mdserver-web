...
def get_dir():
    ...
    dir_list["page"] = mw.getPage(
        {"p": page, "row": row, "tojs": "getFiles", "count": dir_list["count"]},
        "1,2,3,4,5,6,7,8",
    )
    return dir_list
...
def upload_segment():
    ...
    return file.uploadSegment(
        path, name, size, start, dir_mode, file_mode, b64_data, upload_files
    )
