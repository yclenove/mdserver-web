...
def login_temp_user(token):
    ...
    mw.writeLog(
        "用户临时登录", "登录成功,帐号:{1},登录IP:{2}", (user_data["name"], login_addr)
    )

    mw.M("temp_login").where("id=?", (tmp_data["id"],)).update(
...
def verifyLogin():
    ...
    two_step_verification = thisdb.getOptionByJson(
...
def do_login():
    ...
    if info is None:
        msg = mw.getInfo(
            "<a style='color: red'>密码错误</a>,帐号:{1},密码:{2},登录IP:{3}",
